    ...
    for i in range(num_problems):
        while True:
            ...
            if random.choice([True, False]):
                # 加法
                a = random.randint(1, 19)  # 不出现0，最大为19
                b = random.randint(1, 20 - a)  # 保证和不超过20
                problem = f"{a} + {b} = "
            else:
                # 减法
                a = random.randint(2, 20)  # 确保减法的a不小于2
...
def save_problems_to_file(problems, filename="math_20.txt"):
...
if __name__ == "__main__":
    ...
    print(f"{len(math_problems)}道数学题已生成并保存到文件 'math_20.txt'。")

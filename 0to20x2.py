...
def generate_math_problems(num_problems=1000):
    ...
    for i in range(num_problems):
        while True:
            ...
            if first_result < 1 or first_result >= 20:
                continue

            # 第二次运算
            if random.choice([True, False]):
                # 加法
                c = random.randint(1, 20 - first_result)
                second_part = f" + {c}"
            else:
                # 减法
...
def save_problems_to_file(problems, filename="math_20x2.txt"):
...
if __name__ == "__main__":
    ...
    print(f"{len(math_problems)}道数学题已生成并保存到文件 'math_20x2.txt'。")
